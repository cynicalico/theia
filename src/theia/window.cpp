...
void theia::Window::make_context_current() { glfwMakeContextCurrent(handle_); }

bool theia::Window::should_close() const { return glfwWindowShouldClose(handle_) == GLFW_TRUE; }

void theia::Window::set_should_close(bool value) { glfwSetWindowShouldClose(handle_, value ? GLFW_TRUE : GLFW_FALSE); }

void theia::Window::swap_buffers() { glfwSwapBuffers(handle_); }
...
void theia::Window::set_size(glm::ivec2 size) { glfwSetWindowSize(handle_, size.x, size.y); }
...
void theia::Window::set_aspect_ratio(int numer, int denom) { glfwSetWindowAspectRatio(handle_, numer, denom); }
...
void theia::Window::set_position(glm::ivec2 pos) { glfwSetWindowPos(handle_, pos.x, pos.y); }

int theia::Window::x() const { return position().x; }

int theia::Window::y() const { return position().y; }

const char *theia::Window::title() const { return glfwGetWindowTitle(handle_); }

void theia::Window::set_title(const std::string &title) { glfwSetWindowTitle(handle_, title.c_str()); }
...
GLFWmonitor *theia::Window::monitor() const { return glfwGetWindowMonitor(handle_); }
...
float theia::Window::opacity() const { return glfwGetWindowOpacity(handle_); }

void theia::Window::set_opacity(float opacity) { glfwSetWindowOpacity(handle_, opacity); }

bool theia::Window::focused() const { return glfwGetWindowAttrib(handle_, GLFW_FOCUSED) == GLFW_TRUE; }

bool theia::Window::iconified() const { return glfwGetWindowAttrib(handle_, GLFW_ICONIFIED) == GLFW_TRUE; }

bool theia::Window::maximized() const { return glfwGetWindowAttrib(handle_, GLFW_MAXIMIZED) == GLFW_TRUE; }

bool theia::Window::hovered() const { return glfwGetWindowAttrib(handle_, GLFW_HOVERED) == GLFW_TRUE; }

bool theia::Window::visible() const { return glfwGetWindowAttrib(handle_, GLFW_VISIBLE) == GLFW_TRUE; }

bool theia::Window::resizable() const { return glfwGetWindowAttrib(handle_, GLFW_RESIZABLE) == GLFW_TRUE; }

bool theia::Window::decorated() const { return glfwGetWindowAttrib(handle_, GLFW_DECORATED) == GLFW_TRUE; }

bool theia::Window::auto_iconify() const { return glfwGetWindowAttrib(handle_, GLFW_AUTO_ICONIFY) == GLFW_TRUE; }

bool theia::Window::floating() const { return glfwGetWindowAttrib(handle_, GLFW_FLOATING) == GLFW_TRUE; }

bool theia::Window::transparent_framebuffer() const {
    return glfwGetWindowAttrib(handle_, GLFW_TRANSPARENT_FRAMEBUFFER) == GLFW_TRUE;
}

bool theia::Window::focused_on_show() const { return glfwGetWindowAttrib(handle_, GLFW_FOCUS_ON_SHOW) == GLFW_TRUE; }

bool theia::Window::mouse_passthrough() const {
    return glfwGetWindowAttrib(handle_, GLFW_MOUSE_PASSTHROUGH) == GLFW_TRUE;
}

void theia::Window::focus() { glfwFocusWindow(handle_); }

void theia::Window::request_focus() { glfwRequestWindowAttention(handle_); }

void theia::Window::iconify() { glfwIconifyWindow(handle_); }

void theia::Window::maximize() { glfwMaximizeWindow(handle_); }

void theia::Window::restore() { glfwRestoreWindow(handle_); }
...
void *theia::Window::user_pointer() const { return glfwGetWindowUserPointer(handle_); }

void theia::Window::set_user_pointer(void *ptr) { glfwSetWindowUserPointer(handle_, ptr); }

GLFWwindow *theia::Window::handle() const { return handle_; }
